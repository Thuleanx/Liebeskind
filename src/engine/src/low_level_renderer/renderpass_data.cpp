...
RenderPassData RenderPassData::create(
	...
	const vk::RenderPass mainPass =
		createMainRenderPass(device, colorAttachmentFormat, swapchainDepthFormat, msaaSampleCount);
	const vk::RenderPass postProcessingPass = createPostProcessingRenderPass(device, swapchainImageFormat.format);
...
vk::RenderPass createMainRenderPass(
	...
	const vk::RenderPassCreateInfo renderPassInfo(
		{}, attachments.size(), attachments.data(), 1, &subpass, 1, &subpassDependency
	);
...
vk::RenderPass createPostProcessingRenderPass(vk::Device device, vk::Format swapchainImageFormat) {
	const std::array<vk::AttachmentDescription, 1> attachments = {vk::AttachmentDescription{
	...
	const vk::SubpassDescription subpass(
		{}, vk::PipelineBindPoint::eGraphics, 0, nullptr, subpassColor.size(), subpassColor.data(), nullptr, nullptr
	);
	const vk::SubpassDependency subpassDependency(
		vk::SubpassExternal,
		0,
